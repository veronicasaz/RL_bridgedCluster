...
from TestEnvironment import calculate_errors
...
def plot_trajs(env, STATES, CONS, Titles, filenames, save_path = None):
    ...
    gs1 = matplotlib.gridspec.GridSpec(10, 3, 
    ...
    for case in range(len(STATES)):
        columns = 2*(case//3)
        ax1 = fig.add_subplot(gs1[columns:columns+2, case%3])
    ...
    # Plot actions of RL
    ax4 = fig.add_subplot(gs1[columns+4:columns+6, :])
    AX.append(ax4)
    steps_taken = np.load(env.settings['Integration']['savefile'] + 'RL_steps_taken.npy', allow_pickle=True)
    plot_actions_taken(ax4, x_axis[1:], steps_taken[1:]) # the first action is fixed
    ax4.set_yticks(np.arange(0, env.settings['Integration']['number_actions']))
...
def run_bridge_vs_no_bridge(env, seed, steps):
    ...
    env.bridged = True
    env.integrator = 'Hermite'
    env.t_step_integr = [1e-6, 1e-6] # smaller time-step parameter for the one with the planets
    run_trajectory(seed = seed, action = 0, env = env,\
...
def prepare_plot_bridge_vs_no_bridge(env, steps):
        ...
        plot_trajs(env,\
                   [state_nobridge_Hermite, state_nobridge_Huayno, state_bridge_fast, state_bridge_mid, state_bridge_accurate, state_bridge_RL], 
                   [], ['No bridge Hermite', 'No bridge Huayno', r'Bridge fast', r'Bridge mid', r'Bridge accurate', r'Bridge RL'],\
                    ['_nobridge_Hermite', '_nobridge_Huayno', '_bridge_fast','_bridge_mid', '_bridge_accurate', '_bridge_RL'], save_path = path_save)
