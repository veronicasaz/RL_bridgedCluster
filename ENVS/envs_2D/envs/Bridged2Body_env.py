...
class TwoBody_env(gym.Env):
    def __init__(self, integrator = 'Hermite', subfolder = '', suffix = ''):
        """
    ...
    def reset(self, seed = None, steps = None, typereward = None, save_state = None):
        ...
        if self.bridged == True:
            ...
            self.channel = [self.grav_global.new_channel_to(self.particles_global), \
                            self.grav_local.new_channel_to(self.particles_local)]

            particles_joined = self.join_particles_bridge([self.particles_global, self.particles_local])
        else:
